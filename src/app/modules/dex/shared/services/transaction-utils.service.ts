...
import { getDefaultFees } from '@xchainjs/xchain-cosmos';
...
export class TransactionUtilsService {
  ...
  calculateNetworkFee(
    ...
    if (matchingInboundAddress) {
      switch (asset.chain) {
        case 'GAIA':
          return getDefaultFees().average.amount().toNumber();
        case 'BTC':
        case 'LTC':
        case 'DOGE':
        case 'BCH':
        ...
        case 'ETH':
          ...
          if (asset.symbol === 'ETH') {
            ...
            return (
              (limit *
                (+matchingInboundAddress.gas_rate * 10 ** 9) *
                multiplier) /
              10 ** 18
            );
          }
          // ERC20
            ...
            if (this._ethPool) {
              const ethGasVal =
                (70000 *
                  +matchingInboundAddress.gas_rate *
                  10 ** 9 *
                  multiplier) /
                10 ** 18;
              // const tokenEthValue = +this._ethPool / +assetPool.assetPriceUSD;
              const tokenEthValue =
      ...
      return direction === 'OUTBOUND'
        ? 0.06
        : this._outboundTransactionFee ?? 0.2;
    } else {
