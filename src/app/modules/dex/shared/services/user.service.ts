...
import { Asset, checkSummedAsset } from '../classes/asset';
import { AssetAndBalance } from '../classes/asset-and-balance';
import { PoolAddressDTO } from '../classes/pool-address';

import { MidgardService } from './midgard.service';

import { TransactionUtilsService } from './transaction-utils.service';
import { TxType } from '../constants/tx-type';
import {
  ETH_BLOCKLIST,
  addressIsBlockListed,
} from '@dexShared/constants/blocklist';
...
export class UserService {
  ...
  setUser(user: User) {
    this._user = user;
    const users = this.userSource.value;
    if (users.filter((us) => us.type == user.type).length == 0) {
      users.push(user);
    }
  ...
  /*async fetchBalances(): Promise<void> {
    ...
    if (
      ...
      this.getEthereumBalances(userEthereumClient, this._user.address);
    }
  }*/
  ...
  async getGeneralBalance(key: ClientType) {
    try {
  ...
  async getBinanceBalances() {
    try {
      ...
      const balances = bncBalances.map((balance: any) => {
  ...
  async getEthereumBalances(client: EthClient, address: string) {
    try {
      ...
      const ethBalance = await provider.getBalance(address); // PROSESS.ENV
      consoleLog('NOPASS');
      this.pushBalances([
        {
  ...
  getTokenAddress(user: User, chain: Chain): string {
    if (user.type === 'metamask') {
      ...
      if (chain == 'BNB') {
        ...
        const dogeClient = clients.dogecoin;
