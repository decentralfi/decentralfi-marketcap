...
import { TCAbi, TCRopstenAbi } from '../abi/thorchain.abi';
import { baseAmount } from '@xchainjs/xchain-util';
import BigNumber from 'bignumber.js';
import { erc20ABI } from '../abi/erc20.abi';
import { environment } from 'src/environments/environment';
import { ethRUNERopsten } from '../abi/erc20RUNE.abi';
...
// prettier-ignore
const testnetBasketABI = [{"inputs":[] as any[],"stateMutability":"nonpayable","type":"constructor"},{"inputs":[{"internalType":"address","name":"coin","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"addCoin","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"coins","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},{"inputs":[],"name":"giveMeCoins","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address","name":"","type":"address"}],"name":"isAdded","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"view","type":"function"}];
...
export class EthUtilsService {
  ...
  async getAssetDecimal(asset: Asset, client: EthClient): Promise<number> {
    if (asset.chain === 'ETH') {
      if (asset.symbol === 'ETH') {
        return ETH_DECIMAL;
      } else {
  ...
  async getTestnetRune(ethClient: EthClient) {
    try {
      ...
      consoleLog('error getting testnet RUNE');
      consoleLog(error);
    }
  }

  async estimateERC20Time(token: string, tokenAmount: number): Promise<number> {
    const tokenPool = await this.midgardService
      .getPoolDetails(token)
      .toPromise();
    const ethPool = await this.midgardService
      .getPoolDetails('ETH.ETH')
      .toPromise();
    // prettier-ignore
